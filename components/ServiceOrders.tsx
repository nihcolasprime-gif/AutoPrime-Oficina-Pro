...
export const ServiceOrders = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        addServiceOrder({
            ...
            // Generate IDs for new services if not present
            servicos: selectedServices.map(s => ({ ...s, id: s.id || Date.now().toString() + Math.random() })),
                                        ...
                                        <div>
                                            ...
                                            <ul className="text-xs text-slate-600 space-y-1">
                                                {os.servicos.slice(0, 3).map((s, i) => (
                                                    <li key={`${os.id}-svc-${i}`} className="truncate">• {s.nome}</li>
                                                ))}
                                        ...
                                        <div>
                                            ...
                                            <ul className="text-xs text-slate-600 space-y-1">
                                                {os.pecasUsadas.slice(0, 3).map((p, i) => (
                                                    <li key={`${os.id}-part-${p.partId}-${i}`} className="truncate">• {p.nomePeca} ({p.quantidade})</li>
                                                ))}
